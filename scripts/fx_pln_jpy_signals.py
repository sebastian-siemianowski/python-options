...
_TUNED_KALMAN_PARAMS_CACHE = None
...
def _load_tuned_kalman_params(asset_symbol: str, cache_path: str = "cache/kalman_q_cache.json") -> Optional[Dict]:
    ...
    """
    global _TUNED_KALMAN_PARAMS_CACHE
    if _TUNED_KALMAN_PARAMS_CACHE is None:
        try:
            if not os.path.exists(cache_path):
                _TUNED_KALMAN_PARAMS_CACHE = {}
            else:
                with open(cache_path, 'r') as f:
                    _TUNED_KALMAN_PARAMS_CACHE = json.load(f)
        except Exception as e:
            if os.getenv('DEBUG'):
                print(f"Warning: Failed to load tuned params for {asset_symbol}: {e}")
            _TUNED_KALMAN_PARAMS_CACHE = {}

    if asset_symbol in _TUNED_KALMAN_PARAMS_CACHE:
        # Direct lookup
        data = _TUNED_KALMAN_PARAMS_CACHE[asset_symbol]
        q_val = data.get('q')
...
def _kalman_filter_drift(ret: pd.Series, vol: pd.Series, q: Optional[float] = None, optimize_q: bool = True, asset_symbol: Optional[str] = None) -> Dict[str, pd.Series]:
    ...
    # Determine q or c to use, and whether to optimize
    q_optimization_attempted = False
    c_optimal = None
    q_t_series = None  # Will store time-varying q_t if heteroskedastic
    tuned_params = _load_tuned_kalman_params(asset_symbol) if asset_symbol else None
    ...
    if tuned_params:
        ...
        optimize_q = False  # Don't optimize if we loaded params
        if os.getenv('DEBUG'):
            print(f"  Using tuned params from cache: q={q:.2e}, c={c_optimal:.3f}")
    elif asset_symbol:
        # If not in cache, do not optimize, use heuristic fallback.
        optimize_q = False
    ...
    # Pass through cached metadata like PIT calibration warning
    if tuned_params:
        result["calibration_warning"] = tuned_params.get("calibration_warning")
        result["pit_ks_pvalue"] = tuned_params.get("pit_ks_pvalue")
    return result
...
def latest_signals(feats: Dict[str, pd.Series], horizons: List[int], last_close: float, t_map: bool = True, ci: float = 0.68) -> Tuple[List[Signal], Dict[int, Dict[str, float]]]: 
    ...
    for H in horizons:
        ...
        # Reduce confidence if model is miscalibrated (PIT warning) or worse than baseline
        kalman_metadata = feats.get("kalman_metadata", {})
        delta_ll_zero = kalman_metadata.get("delta_ll_vs_zero", float("nan"))
        calibration_warning = kalman_metadata.get("calibration_warning", False)

        drift_weight = 1.0
        if np.isfinite(delta_ll_zero) and delta_ll_zero < 0:
            drift_weight = 0.0  # Model is worse than zero-drift, don't trust drift.
        elif calibration_warning:
            drift_weight = 0.5  # Model is miscalibrated, reduce confidence by 50%.

        pos_strength *= drift_weight
