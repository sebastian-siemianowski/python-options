...
# When enabled:
# - Uses only cached price data from src/data/prices
...
FX_RATE_CACHE_PATH = os.path.join("src", "data", "currencies", "fx_rates.json")
...
def load_failed_assets() -> Dict[str, Dict]:
    """Load failed assets from src/data/failed/failed_assets.json.
...
def purge_failed_assets_from_cache(verbose: bool = True) -> Dict[str, bool]:
    ...
    if not failures:
        if verbose:
            print("No failed assets found in src/data/failed/failed_assets.json")
        return {}
...
def drop_first_k_from_kalman_cache(k: int = 4, cache_path: str = 'src/data/tune') -> List[str]:
    """
