...
if [[ -x ".venv/bin/python" ]]; then
  ...
  if [[ -z "$PY_CMD" ]]; then
    echo "Error: Could not find a working Python 3 interpreter. Install Python 3 or create the venv with: bash src/setup/setup_venv.sh" >&2
    exit 1
  fi
fi
...
exec "$PY_CMD" src/options/options.py --min_oi "$MIN_OI" --min_vol "$MIN_VOL" "$@"
