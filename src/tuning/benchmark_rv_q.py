#!/usr/bin/env python3
"""Focused A/B benchmark for RV-Q optimizer precomputed volatility inputs."""
...
from models.rv_adaptive_q import optimize_rv_q_params  # noqa: E402
...
def _worker(args: tuple[str, bool, str]) -> Dict[str, Any]:
    symbol, disable_precomputed, model = args
    if disable_precomputed:
        os.environ["RV_Q_DISABLE_PRECOMPUTED_KERNEL"] = "1"
        os.environ.pop("RV_Q_ENABLE_PRECOMPUTED_KERNEL", None)
    else:
        os.environ.pop("RV_Q_DISABLE_PRECOMPUTED_KERNEL", None)
        os.environ["RV_Q_ENABLE_PRECOMPUTED_KERNEL"] = "1"
    returns, vol = _prepare_asset(symbol)
    nu = 8.0 if model == "student_t" else None
    t0 = time.perf_counter()
    config, diagnostics = optimize_rv_q_params(
        returns, vol, c=1.2, phi=0.65, nu=nu, train_frac=0.7
    )
    elapsed = time.perf_counter() - t0
    return {
        "symbol": symbol,
        "seconds": elapsed,
        "q_base": float(config.q_base),
        "gamma": float(config.gamma),
        "log_likelihood": float(diagnostics["log_likelihood"]),
        "delta_bic": float(diagnostics["delta_bic"]),
        "oos_delta_ll": float(diagnostics.get("oos_delta_ll", 0.0)),
    }


def _run(label: str, assets: List[str], workers: int, disable: bool, model: str) -> Dict[str, Any]:
    t0 = time.perf_counter()
    results: Dict[str, Dict[str, Any]] = {}
    failures: Dict[str, str] = {}
    with ProcessPoolExecutor(max_workers=workers) as pool:
        futs = {pool.submit(_worker, (sym, disable, model)): sym for sym in assets}
    ...
    return {
        "label": label,
        "disable_precomputed_kernel": disable,
        "workers": workers,
        "model": model,
...
def main() -> None:
    ...
    parser.add_argument("--model", choices=["gaussian", "student_t"], default="student_t")
    parser.add_argument("--metrics-json", default="src/data/benchmarks/rv_q_metrics.json")
    ...
    before = _run("before_precomputed_kernel", assets, workers, True, args.model)
    after = _run("after_precomputed_kernel", assets, workers, False, args.model)
    ...
    for sym, after_result in after["results"].items():
        ...
        paired.append({
            ...
            "q_base_delta": before_result["q_base"] - after_result["q_base"],
            "gamma_delta": before_result["gamma"] - after_result["gamma"],
            "log_likelihood_delta": before_result["log_likelihood"] - after_result["log_likelihood"],
            "delta_bic_delta": before_result["delta_bic"] - after_result["delta_bic"],
            "oos_delta_ll_delta": before_result["oos_delta_ll"] - after_result["oos_delta_ll"],
        })
    ...
    summary = {
        ...
        "max_abs_q_base_delta": max((abs(p["q_base_delta"]) for p in paired), default=None),
        "max_abs_gamma_delta": max((abs(p["gamma_delta"]) for p in paired), default=None),
        "max_abs_log_likelihood_delta": max((abs(p["log_likelihood_delta"]) for p in paired), default=None),
        "max_abs_delta_bic_delta": max((abs(p["delta_bic_delta"]) for p in paired), default=None),
        "max_abs_oos_delta_ll_delta": max((abs(p["oos_delta_ll_delta"]) for p in paired), default=None),
    }
