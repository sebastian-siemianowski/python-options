...
class TestPhiSkewTModel:
    ...
    def test_skew_t_sampling(self):
        ...
        Fernández-Steel convention:
          gamma < 1 → right-skewed (positive values amplified: z/gamma)
          gamma > 1 → left-skewed (negative values amplified: z*gamma)
        """
        from models import PhiSkewTDriftModel
        
        rng = np.random.default_rng(42)
        
        # gamma < 1 → right-skewed in Fernández-Steel convention
        ...
        assert skew_right > 0, f"gamma<1 should produce positive skewness, got {skew_right}"
        ...
        assert skew_left < 0, f"gamma>1 should produce negative skewness, got {skew_left}"
        ...
        assert abs(skew_sym) < 0.15, f"Symmetric should have ~0 skewness, got {skew_sym}"
    ...
    def test_skew_t_pit_calibration(self):
        ...
        # Note: filter dynamics diverge from sampling DGP, so PIT may not be ideal
        assert ks_p >= 0 and ks_stat <= 1, f"Invalid KS stat/p: {ks_stat}, {ks_p}"
    ...
    def test_cdf_properties(self):
        ...
        # CDF should be approximately monotone increasing (allow numerical noise from quadrature)
        x_vals = np.linspace(-5, 5, 100)
        cdf_vals = [PhiSkewTDriftModel.cdf_skew_t(x, nu=6, gamma=0.85) for x in x_vals]
        assert all(cdf_vals[i] <= cdf_vals[i+1] + 1e-6 for i in range(len(cdf_vals)-1)), "CDF not monotone"
...
class TestBMAIntegration:
    ...
    def test_model_imports_skew_t(self):
        """Test that model module exports skew-t components."""
        from models import (
            PhiSkewTDriftModel,
            SKEW_T_NU_GRID,
            SKEW_T_GAMMA_GRID,
    ...
    def test_is_skew_t_model_recognizes_skew_t(self):
        """Test that is_skew_t_model recognizes skew-t models."""
        from models import is_skew_t_model
        ...
        assert is_skew_t_model("phi_student_t_nu_6") == False
        assert is_skew_t_model("kalman_gaussian") == False
        assert is_skew_t_model("kalman_phi_gaussian") == False
