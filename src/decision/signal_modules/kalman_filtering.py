    ...
    if tuned_params is not None and tuned_params.get('gaussian_unified') and gas_q_result is None:
        try:
            ...
            g_unified_config = GaussianUnifiedConfig(
                ...
                chisq_ewm_lambda=float(tuned_params.get('chisq_ewm_lambda', 0.98)),
                pit_var_lambda=float(tuned_params.get('pit_var_lambda', 0.97)),
                pit_var_dz_lo=float(tuned_params.get('pit_var_dz_lo', 0.30)),
                pit_var_dz_hi=float(tuned_params.get('pit_var_dz_hi', 0.55)),
