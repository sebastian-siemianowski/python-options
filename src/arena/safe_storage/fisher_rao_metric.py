"""
Fisher-Rao Metric Model - Standalone
====================================
Fisher information metric for adaptive step sizing.

The Fisher-Rao metric g_ij = E[d_i log p * d_j log p] defines
the natural geometry on the space of probability distributions.

For Gaussian: I(sigma^2) = 1/(2*sigma^4)

We compute running Fisher information and use it to modulate
the Kalman gain: when I is high (low variance, high info), 
reduce gain to prevent overconfidence.
"""
import numpy as np
...
class FisherRaoMetricModel:
    ...
    def _fisher_info(self, vol, t):
        """Compute running Fisher information estimate."""
        if t < self.FISHER_WINDOW:
            return 1.0
        w = vol[t - self.FISHER_WINDOW:t]
        w = w[w > 0]
        if len(w) < 10:
            return 1.0
        var_est = np.var(w) + 1e-10
        fisher = 1.0 / (2.0 * var_est ** 2)
        log_fisher = np.log1p(fisher)
        self._fisher_ema = self.FISHER_DAMPING * self._fisher_ema + (1 - self.FISHER_DAMPING) * log_fisher
        normalized = np.clip(self._fisher_ema / (np.log1p(1.0 / (2.0 * 0.01 ** 2)) + 1e-8), 0.3, 2.5)
        return normalized
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._fisher_ema = 1.0
        rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            fisher_mod = self._fisher_info(vol, t)
            mult = defl * hcorr
            pm = phi * state
            pv = phi**2 * P + q * mult * stress
            blend = 0.54 * rv + 0.46 * ema_vol
            obs = c * blend * mult * np.sqrt(ent * stress) * np.sqrt(fisher_mod)
            S = pv + obs**2
