"""
RG Fixed Point Model - Standalone
=================================
Fixed-point iteration for optimal deflation parameter.

RG fixed points g* satisfy beta(g*) = 0. Near a fixed point:
  g(L) = g* + C * L^{-1/nu}  (relevant direction)

We treat the deflation parameter as a "coupling constant" and 
iterate toward its fixed point using successive approximation:
  defl_{n+1} = T(defl_n) where T is the RG transformation

The fixed point is self-consistent: defl* gives the target Hyvärinen.
"""
import numpy as np
...
class RGFixedPointModel:
    ...
    def _fixed_point_deflation(self, running_hyv, hyv_count):
        """Iterate toward fixed-point deflation."""
        if hyv_count < 20:
            return self._fp_defl
        avg_hyv = running_hyv / hyv_count
        for _ in range(self.FP_ITERATIONS):
            correction = self.FP_ALPHA * (avg_hyv - self.HYV_TARGET) / (abs(self.HYV_TARGET) + 1e-8)
            self._fp_defl = self._fp_defl * (1.0 - correction)
            self._fp_defl = np.clip(self._fp_defl, 0.4, 1.8)
        return self._fp_defl
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._fp_defl = 1.0
        rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            fp_defl = self._fixed_point_deflation(rh, hc)
            mult = defl * hcorr * (0.7 + 0.3 * fp_defl)
