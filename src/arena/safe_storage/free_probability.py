"""
Free Probability Model - Standalone
===================================
Free convolution: eigenvalue distribution of sum of random matrices.

Implements concepts from free probability theory where we track
the eigenvalue distribution of sums of random matrices formed
from return windows. The Marchenko-Pastur ratio indicates
departure from random matrix behavior.
"""
import numpy as np
...
class FreeProbabilityModel:
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._fpr = 1.0
        rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            custom_mod = self._fp_mod(ret, vol, t, mags_t)
            mult = defl * hcorr
