"""
Adjoint Duality Model - Standalone
==================================
Adjoint functor pair: free-forgetful on volatility space.

Implements categorical duality between "free" and "forgetful"
transformations of returns relative to volatility. The adjoint
gap measures asymmetry in these dual views.
"""
import numpy as np
...
class AdjointDualityModel:
    ...
    def _adj_mod(self, returns, vol, t, mags=None):
        if t < 40: return 1.0
        ret_w = returns[t-40:t]
        vol_w = vol[t-40:t]
        vol_w = np.maximum(vol_w, 1e-8)
        free = ret_w / vol_w
        forgetful = ret_w * vol_w
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._ad = 1.0
        rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            custom_mod = self._adj_mod(ret, vol, t, mags_t)
            mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6))
            S = pv + obs**2
