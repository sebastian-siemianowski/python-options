"""
Martingale Stopping Model - Standalone
======================================
Doob's optional stopping: E[M_tau] = E[M_0]. 

Track cumulative innovation as martingale. When |M_t| exceeds 
Doob's maximal inequality bound, inflate S.

The martingale property ensures that the expected value remains
constant at stopping times, providing a theoretical foundation
...
class MartingaleStoppingModel:
    ...
    def _filter(self, ret, vol, p):
        ...
        self._mart_sum = 0.0; self._mart_sq = 0.0
        rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            mart_mod = 1.0
            if t > 30:
                doob_bound = np.sqrt(self._mart_sq / t + 1e-10) * 2.0
                if abs(self._mart_sum) > doob_bound and doob_bound > 0:
                    mart_mod = 1.0 + 0.15 * (abs(self._mart_sum) / doob_bound - 1.0)
                    mart_mod = min(mart_mod, 1.6)
            mult = defl * hcorr
            pm = phi * state
            pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*mart_mod)
            S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10))
            inn = ret[t]-pm
            self._mart_sum += inn / (sigma[t] + 1e-10)
            self._mart_sq += (inn / (sigma[t] + 1e-10))**2
