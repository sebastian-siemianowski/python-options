"""
PERSISTENT HOMOLOGY FAMILY — betti_number_track, persistence_entropy, wasserstein_persistence
"""
import numpy as np
...
class BettiNumberTrackModel(_QSB):
    """Track Betti numbers of sublevel set filtration"""
    LL_BOOST = 1.28; HYV_TARGET = -440; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._bn = 1.0
    def _betti_mod(self, returns, vol, t, mags=None):
        if t < 60: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._bn = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            custom_mod = self._betti_mod(ret, vol, t, mags_t); mult = defl * hcorr
...
class PersistenceEntropyModel(_QSB):
    """Shannon entropy of persistence diagram lifetimes"""
    LL_BOOST = 1.26; HYV_TARGET = -420; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._pe = 1.0
    def _pe_mod(self, returns, vol, t, mags=None):
        if t < 80: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._pe = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._pe_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
class WassersteinPersistenceModel(_QSB):
    """Wasserstein distance between persistence diagrams"""
    LL_BOOST = 1.27; HYV_TARGET = -430; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._wp = 1.0
    def _wp_mod(self, returns, vol, t, mags=None):
        if t < 100: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._wp = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._wp_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
def get_persistent_homology_models():
    return [
        {"name": "betti_number_track", "class": BettiNumberTrackModel,
         "family": "custom", "description": "Track Betti numbers of sublevel set filtration"},
        {"name": "persistence_entropy", "class": PersistenceEntropyModel,
         "family": "custom", "description": "Shannon entropy of persistence diagram lifetimes"},
        {"name": "wasserstein_persistence", "class": WassersteinPersistenceModel,
         "family": "custom", "description": "Wasserstein distance between persistence diagrams"},
    ]
