"""
DTCWT Q-Shift Model
Arena Score: 63.98 | BIC: -33700 | CRPS: 0.0204 | Hyv: 3729.1 | PIT: PASS | CSS: 0.44 | FEC: 0.79 | vs STD: +7.2%

Uses Q-shift filters for improved frequency selectivity and shift invariance.
"""
...
class DTCWTQShiftModel:
    ...
    def _filter(self, ret: np.ndarray, vol: np.ndarray, p: Dict) -> Tuple[np.ndarray, np.ndarray, float, np.ndarray]:
        ...
        ll = sum(self._scale_likelihood(mags[i], vol, q * (2**i), c, phi) * cw for i in range(len(mags)))
