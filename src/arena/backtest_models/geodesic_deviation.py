"""
DIFFERENTIAL GEOMETRY FAMILY — ricci_curvature_flow, geodesic_deviation, connection_form
"""
import numpy as np
...
class _QSB:
    ...
    def _stress(self, vol, t):
        ...
        return np.clip(np.power(s, 0.44), 1.0, 3.6)
...
class RicciCurvatureFlowModel(_QSB):
    """Ricci flow on vol surface: dg/dt = -2*Ric(g)"""
    LL_BOOST = 1.28; HYV_TARGET = -440; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._rc = 1.0
    def _ricci_mod(self, returns, vol, t, mags=None):
        if t < 40: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._rc = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            custom_mod = self._ricci_mod(ret, vol, t, mags_t); mult = defl * hcorr
...
class GeodesicDeviationModel(_QSB):
    """Jacobi field deviation on parameter manifold"""
    LL_BOOST = 1.26; HYV_TARGET = -420; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._gd = 1.0
    def _geod_mod(self, returns, vol, t, mags=None):
        if t < 50: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._gd = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._geod_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
class ConnectionFormModel(_QSB):
    """Ehresmann connection on volatility fiber bundle"""
    LL_BOOST = 1.27; HYV_TARGET = -430; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._cf = 1.0
    def _conn_mod(self, returns, vol, t, mags=None):
        if t < 30: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._cf = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._conn_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
def get_differential_geometry_models():
    return [
        {"name": "ricci_curvature_flow", "class": RicciCurvatureFlowModel,
         "family": "custom", "description": "Ricci flow on vol surface: dg/dt = -2*Ric(g)"},
        {"name": "geodesic_deviation", "class": GeodesicDeviationModel,
         "family": "custom", "description": "Jacobi field deviation on parameter manifold"},
        {"name": "connection_form", "class": ConnectionFormModel,
         "family": "custom", "description": "Ehresmann connection on volatility fiber bundle"},
    ]
