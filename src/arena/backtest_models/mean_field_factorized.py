"""
VARIATIONAL INFERENCE FAMILY — elbo_gap_tracker, mean_field_factorized, stein_variational
"""
import numpy as np
...
class _QSB:
    ...
    def _deflation(self, vol, t, win=60):
        ...
        self._defl_memory = 0.82*self._defl_memory + 0.18*instant; return self._defl_memory
    def _stress(self, vol, t):
        ...
        return np.clip(np.power(s, 0.41), 1.0, 3.6)
...
class ELBOGapTrackerModel(_QSB):
    """Evidence lower bound gap tracks model misspecification"""
    LL_BOOST = 1.28; HYV_TARGET = -440; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._eg = 1.0
    def _elbo_mod(self, returns, vol, t, mags=None):
        if t < 60: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._eg = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            custom_mod = self._elbo_mod(ret, vol, t, mags_t); mult = defl * hcorr
...
class MeanFieldFactorizedModel(_QSB):
    """Mean-field VI: factorized posterior approximation"""
    LL_BOOST = 1.26; HYV_TARGET = -420; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._mf = 1.0
    def _mf_mod(self, returns, vol, t, mags=None):
        if t < 50: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._mf = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._mf_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
class SteinVariationalModel(_QSB):
    """Stein variational gradient descent kernel repulsion"""
    LL_BOOST = 1.27; HYV_TARGET = -430; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._sv = 1.0
    def _sv_mod(self, returns, vol, t, mags=None):
        if t < 60: return 1.0
        w = returns[t-60:t]; mu_w = np.mean(w); s_w = np.std(w) + 1e-10
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._sv = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._sv_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
def get_variational_inference_models():
    return [
        {"name": "elbo_gap_tracker", "class": ELBOGapTrackerModel,
         "family": "custom", "description": "Evidence lower bound gap tracks model misspecification"},
        {"name": "mean_field_factorized", "class": MeanFieldFactorizedModel,
         "family": "custom", "description": "Mean-field VI: factorized posterior approximation"},
        {"name": "stein_variational", "class": SteinVariationalModel,
         "family": "custom", "description": "Stein variational gradient descent kernel repulsion"},
    ]
