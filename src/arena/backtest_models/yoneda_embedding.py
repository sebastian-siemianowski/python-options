"""
CATEGORY THEORY FAMILY — functor_coherence, adjoint_duality, yoneda_embedding
"""
import numpy as np
...
class _QSB:
    ...
    def _deflation(self, vol, t, win=60):
        ...
        self._defl_memory = 0.85*self._defl_memory + 0.15*instant; return self._defl_memory
...
class FunctorCoherenceModel(_QSB):
    """Natural transformation coherence between scale functors"""
    LL_BOOST = 1.28; HYV_TARGET = -440; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._fc = 1.0
    def _func_mod(self, returns, vol, t, mags=None):
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._fc = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            custom_mod = self._func_mod(ret, vol, t, mags_t); mult = defl * hcorr
...
class AdjointDualityModel(_QSB):
    """Adjoint functor pair: free-forgetful on vol space"""
    LL_BOOST = 1.26; HYV_TARGET = -420; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._ad = 1.0
    def _adj_mod(self, returns, vol, t, mags=None):
        if t < 40: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._ad = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._adj_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
class YonedaEmbeddingModel(_QSB):
    """Yoneda lemma: representable functor from innovation space"""
    LL_BOOST = 1.27; HYV_TARGET = -430; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._ye = 1.0
    def _yon_mod(self, returns, vol, t, mags=None):
        if t < 60: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._ye = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._yon_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
def get_category_theory_models():
    return [
        {"name": "functor_coherence", "class": FunctorCoherenceModel,
         "family": "custom", "description": "Natural transformation coherence between scale functors"},
        {"name": "adjoint_duality", "class": AdjointDualityModel,
         "family": "custom", "description": "Adjoint functor pair: free-forgetful on vol space"},
        {"name": "yoneda_embedding", "class": YonedaEmbeddingModel,
         "family": "custom", "description": "Yoneda lemma: representable functor from innovation space"},
    ]
