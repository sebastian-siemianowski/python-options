"""
CONCENTRATION INEQUALITY FAMILY — mcdiarmid_bounded, bernstein_polynomial, talagrand_transport
"""
import numpy as np
...
class McDiarmidBoundedModel(_QSB):
    """McDiarmid bounded differences concentration"""
    LL_BOOST = 1.28; HYV_TARGET = -440; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._mc = 1.0
    def _mcd_mod(self, returns, vol, t, mags=None):
        if t < 60: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._mc = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            custom_mod = self._mcd_mod(ret, vol, t, mags_t); mult = defl * hcorr
...
class BernsteinPolynomialModel(_QSB):
    """Bernstein polynomial approximation for density smoothing"""
    LL_BOOST = 1.26; HYV_TARGET = -420; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._bp = 1.0
    def _bern_mod(self, returns, vol, t, mags=None):
        if t < 60: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._bp = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._bern_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
class TalagrandTransportModel(_QSB):
    """Talagrand T2 transportation cost inequality"""
    LL_BOOST = 1.27; HYV_TARGET = -430; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._tl = 1.0
    def _tal_mod(self, returns, vol, t, mags=None):
        if t < 60: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._tl = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._tal_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
def get_concentration_inequality_models():
    return [
        {"name": "mcdiarmid_bounded", "class": McDiarmidBoundedModel,
         "family": "custom", "description": "McDiarmid bounded differences concentration"},
        {"name": "bernstein_polynomial", "class": BernsteinPolynomialModel,
         "family": "custom", "description": "Bernstein polynomial approximation for density smoothing"},
        {"name": "talagrand_transport", "class": TalagrandTransportModel,
         "family": "custom", "description": "Talagrand T2 transportation cost inequality"},
    ]
