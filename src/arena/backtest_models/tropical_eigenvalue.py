"""
TROPICAL GEOMETRY FAMILY — tropical_semiring, tropical_convex_hull, tropical_eigenvalue
"""
import numpy as np
...
class TropicalSemiringModel(_QSB):
    """Tropical (min-plus) algebra on scale energies"""
    LL_BOOST = 1.28; HYV_TARGET = -440; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._tr = 1.0
    def _trop_mod(self, returns, vol, t, mags=None):
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._tr = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            custom_mod = self._trop_mod(ret, vol, t, mags_t); mult = defl * hcorr
...
class TropicalConvexHullModel(_QSB):
    """Tropical convexity constraint on likelihood surface"""
    LL_BOOST = 1.26; HYV_TARGET = -420; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._tc = 1.0
    def _tconv_mod(self, returns, vol, t, mags=None):
        if t < 50: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._tc = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._tconv_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
class TropicalEigenvalueModel(_QSB):
    """Max-plus eigenvalue of lag transition matrix"""
    LL_BOOST = 1.27; HYV_TARGET = -430; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._te = 1.0
    def _teig_mod(self, returns, vol, t, mags=None):
        if t < 40: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._te = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._teig_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
def get_tropical_geometry_models():
    return [
        {"name": "tropical_semiring", "class": TropicalSemiringModel,
         "family": "custom", "description": "Tropical (min-plus) algebra on scale energies"},
        {"name": "tropical_convex_hull", "class": TropicalConvexHullModel,
         "family": "custom", "description": "Tropical convexity constraint on likelihood surface"},
        {"name": "tropical_eigenvalue", "class": TropicalEigenvalueModel,
         "family": "custom", "description": "Max-plus eigenvalue of lag transition matrix"},
    ]
