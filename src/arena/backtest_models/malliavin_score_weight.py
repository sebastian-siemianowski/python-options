"""
Malliavin Score Weight Model - Standalone
==========================================
Malliavin derivative score weighting for likelihood.

The Malliavin derivative D_t F measures sensitivity of a functional F
to perturbations at time t in the Wiener path. For financial returns:
  D_t(log S_T) = sigma_t / S_t (for GBM)

We approximate the Malliavin weight:
  w_t = E[D_t(innovation^2)] / E[D_t(S)]

This weight tells us how "informative" each observation is about
the underlying Wiener process. High Malliavin weight → observation
is highly sensitive to noise → inflate S.
"""
import numpy as np
...
class MalliavinScoreWeightModel:
    ...
    def _deflation(self, vol, t, win=60):
        ...
        self._defl_memory = 0.84*self._defl_memory + 0.16*instant
    ...
    def _malliavin_weight(self, returns, vol, t):
        """Approximate Malliavin derivative weight."""
        if t < self.MALL_WINDOW:
            return 1.0
        ret_w = returns[t-self.MALL_WINDOW:t]
        vol_w = vol[t-self.MALL_WINDOW:t]
        vol_w = np.maximum(vol_w, 1e-8)
        innovations_sq = ret_w**2
        sensitivities = vol_w / (np.abs(ret_w) + 1e-8)
        mean_sensitivity = np.mean(sensitivities)
        mean_inn_sq = np.mean(innovations_sq) + 1e-10
        mall_ratio = mean_sensitivity / (mean_inn_sq + 1e-8)
        mall_ratio = np.clip(mall_ratio, 0.1, 10.0)
        log_ratio = np.log1p(mall_ratio * self.MALL_SENSITIVITY)
        self._mall_weight = self.MALL_MEMORY * self._mall_weight + (1-self.MALL_MEMORY) * log_ratio
        return np.clip(1.0 + self._mall_weight * 0.5, 0.8, 1.8)
    ...
    def _filter(self, ret, vol, p):
        ...
        P, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._mall_weight = 1.0
        rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            mall_mod = self._malliavin_weight(ret, vol, t)
            mult = defl * hcorr
            pm = phi * state; pv = phi**2*P + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*mall_mod)
            S = pv + obs**2
