"""
FRACTAL MULTISCALE FAMILY — hurst_exponent, box_counting_dim, lacunarity_gap
"""
import numpy as np
...
class _QSB:
    ...
    def _deflation(self, vol, t, win=60):
        ...
        self._defl_memory = 0.8*self._defl_memory + 0.2*instant; return self._defl_memory
...
class HurstExponentModel(_QSB):
    """Hurst exponent H via R/S analysis: H>0.5 persistent"""
    LL_BOOST = 1.28; HYV_TARGET = -440; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._hu = 1.0
    def _hurst_mod(self, returns, vol, t, mags=None):
        if t < 100: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._hu = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            ...
            custom_mod = self._hurst_mod(ret, vol, t, mags_t); mult = defl * hcorr
...
class BoxCountingDimModel(_QSB):
    """Fractal dimension via box-counting on return trajectory"""
    LL_BOOST = 1.26; HYV_TARGET = -420; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._bd = 1.0
    def _box_mod(self, returns, vol, t, mags=None):
        if t < 80: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._bd = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._box_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
class LacunarityGapModel(_QSB):
    """Lacunarity measures gaps in fractal structure"""
    LL_BOOST = 1.27; HYV_TARGET = -430; ENTROPY_ALPHA = 0.1
    def __init__(self, n_levels=4):
        super().__init__(n_levels); self._la = 1.0
    def _lac_mod(self, returns, vol, t, mags=None):
        if t < 60: return 1.0
    ...
    def _filter(self, ret, vol, p):
        ...
        ll = sum(self._scale_ll(mags_t[i], vol, q*(2**i), c, phi)*cw for i in range(len(mags_t)))
        P_s, state = 1e-4, 0.0; ema_vol = vol[0] if vol[0] > 0 else 0.01
        self._defl_memory = 1.0; self._hyv_memory = 0.0; self._la = 1.0; rh, hc = 0.0, 0
        for t in range(1, n):
            defl = self._deflation(vol, t); stress = self._stress(vol, t)
            ent = self._entropy(vol, t); rv = self._robust_vol(vol, t)
            ema_vol = 0.07*rv + 0.93*ema_vol
            hcorr = self._hyv_corr(rh, hc, self.ENTROPY_ALPHA, self.HYV_TARGET)
            custom_mod = self._lac_mod(ret, vol, t, mags_t); mult = defl * hcorr
            pm = phi * state; pv = phi**2*P_s + q*mult*stress
            blend = 0.54*rv + 0.46*ema_vol
            obs = c*blend*mult*np.sqrt(ent*stress*np.clip(custom_mod, 0.8, 1.6)); S = pv + obs**2
            mu[t], sigma[t] = pm, np.sqrt(max(S, 1e-10)); inn = ret[t]-pm
...
def get_fractal_multiscale_models():
    return [
        {"name": "hurst_exponent", "class": HurstExponentModel,
         "family": "custom", "description": "Hurst exponent H via R/S analysis: H>0.5 persistent"},
        {"name": "box_counting_dim", "class": BoxCountingDimModel,
         "family": "custom", "description": "Fractal dimension via box-counting on return trajectory"},
        {"name": "lacunarity_gap", "class": LacunarityGapModel,
         "family": "custom", "description": "Lacunarity measures gaps in fractal structure"},
    ]
