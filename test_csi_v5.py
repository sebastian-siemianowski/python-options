"""
CSI v5: Hybrid Sell Architecture — Oversold-Suppressed Trend + Exhaustion
==========================================================================
v3 problem: sell signals fire at bottoms (mean reversion bounce → wrong direction)
v4 problem: pure exhaustion sell fires too rarely (1% signal frequency)
v5 solution: blend of trend-sell (suppressed at oversold) + exhaustion-sell (amplified at tops)

Key principles:
  - At TOPS: exhaustion signals dominate → CSI goes negative (predicts decline)
  - At BOTTOMS: oversold suppression weakens sell → CSI stays near zero (avoids false sell)
  - During TRENDS: trend-following works normally → CSI follows trend direction
"""
...
def compute_csi_v5(df):
    ...
    # BUY oscillator (oversold bounce)
    ...
    # SELL oscillator (overbought + declining = exhaustion)
    ...
    # ═══════════════════════════════════════════════════════
    # ═══ SELL SIDE (NEW: dual-mode with suppression) ══════
    # ═══════════════════════════════════════════════════════

    # Component A: Trend sell (v3 style — fires during downtrends)
    trend_sell = (
        0.30 * (-trend_score).clip(0, None) +
        0.30 * (-mom_score).clip(0, None) +
        0.20 * (-vol_flow).clip(0, None) +
        0.15 * (-ma_context).clip(0, None) +
        0.05 * (-mom_accel_n).clip(0, None)
    )

    # SUPPRESS trend_sell at oversold levels (this is the KEY fix)
    # At oversold levels, trend_sell fires strongly but prices bounce → wrong signal
    oversold_level = osc_buy.clip(0, 1)
    # Also use broader oversold detection (RSI < 40, Stoch < 35)
    broad_oversold = pd.Series(
        0.5 * np.where(rsi < 40, (40 - rsi) / 40, 0.0) +
        0.5 * np.where(stoch_k < 35, (35 - stoch_k) / 35, 0.0),
        index=close.index
    ).clip(0, 1)
    oversold_suppression = 1.0 - 0.80 * np.maximum(oversold_level, broad_oversold)
    trend_sell = trend_sell * oversold_suppression

    # Component B: Exhaustion sell (fires at TOPS — extended + fading)
    # S1. Extension above MAs
    ext_20 = (close / sma_20 - 1).clip(0, None)
    ext_50 = (close / sma_50 - 1).clip(0, None)
    ...
    # S2. Momentum exhaustion: positive momentum fading
    ...
    # S3. osc_sell (already computed: overbought + declining)
    ...
    vol_weakening = (vol_10ma < vol_20ma * 0.92).astype(float)
    ...
    # Blend: 35% trend sell (suppressed) + 65% exhaustion sell
    sell_raw = 0.35 * trend_sell + 0.65 * exhaust_sell

    # Scale sell to match buy magnitude (exhaustion factors have smaller raw values)
    sell_raw = sell_raw * 2.5
    ...
    sell_ex_factors = pd.concat([
        (extension_score > 0.10).astype(float),
        (mom_exhaust > 0.05).astype(float),
        (osc_sell > 0.05).astype(float),
        (trend_sell > 0.05).astype(float),
        (vol_price_div > 0.10).astype(float),
    ], axis=1).sum(axis=1)
...
def main():
    print("=" * 110)
    print("  CSI v5 — HYBRID SELL: Oversold-Suppressed Trend + Exhaustion")
    print("=" * 110)
    print()
    ...
    print(f"  Avg Signal Frequency:    {safe_mean([r['signal_freq_pct'] for r in results]):.1f}%")
    ...
    print("  ┌────────────────────────┬──────────┬──────────┬──────────┬──────────┐")
    print("  │ Metric                 │    v2    │    v3    │    v4    │  v5 NOW  │")
    print("  ├────────────────────────┼──────────┼──────────┼──────────┼──────────┤")
    print(f"  │ Mean Sell Hit Rate     │  38.9%   │  41.0%   │  39.6%   │ {safe_mean(sell_hits):>5.1f}%   │")
    print(f"  │ Sell signals correct   │  13/50   │  11/50   │   3/11   │ {sell_correct:>2d}/{sell_eligible:<2d}    │")
    print(f"  │ Avg Sell 5d Ret        │ +0.890%  │ +0.867%  │ +1.431%  │{safe_mean(sell_avgs):>+7.3f}%  │")
    print(f"  │ Mean Buy Hit Rate      │  56.1%   │  56.7%   │  56.1%   │ {safe_mean(buy_hits):>5.1f}%   │")
    print(f"  │ Buy signals profitable │  37/50   │  41/50   │  25/48   │ {buy_profitable:>2d}/{buy_eligible:<2d}    │")
    print(f"  │ Sharpe beats B&H       │   9/50   │  14/50   │   9/50   │ {sharpe_beat:>2d}/{sharpe_eligible:<2d}    │")
    print(f"  │ Median Sharpe          │  0.471   │  0.487   │  0.555   │ {safe_median(sharpes):>6.3f}   │")
    print(f"  │ Signal Frequency       │  17.0%   │  17.0%   │   1.0%   │ {safe_mean([r['signal_freq_pct'] for r in results]):>5.1f}%   │")
    print("  └────────────────────────┴──────────┴──────────┴──────────┴──────────┘")
