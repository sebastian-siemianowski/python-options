...
build-russell: .venv/.deps_installed
	@.venv/bin/python src/russell2500.py --out data/universes/russell2500_tickers.csv

russell5000: .venv/.deps_installed
	@.venv/bin/python src/russell5000.py --out data/universes/russell5000_tickers.csv $(ARGS)
...
fx-validate-kalman-plots: .venv/.deps_installed
	...
	@.venv/bin/python src/signals.py --validate-kalman --validation-plots $(ARGS)

# Kalman q parameter tuning via MLE (with world-class UX)
tune: .venv/.deps_installed
	@mkdir -p cache
	@.venv/bin/python src/tune_ux.py $(ARGS)
...
escalate: .venv/.deps_installed
	...
	@.venv/bin/python src/tune_ux.py --force-escalation $(ARGS)
...
calibrate-four: .venv/.deps_installed
	...
	if [ -z "$$FAILED_ASSETS" ]; then \
		...
		.venv/bin/python src/tune_ux.py --assets "$$FAILED_ASSETS" --force $(ARGS); \
	fi
...
stocks: .venv/.deps_installed
	@.venv/bin/python src/refresh_data.py --skip-trim --retries 5 --workers 12 --batch-size 16 $(ARGS)
	@$(MAKE) fx-plnjpy
...
top20: .venv/.deps_installed
	@ASSETS=$$(PYTHONPATH=$(CURDIR) ./.venv/bin/python -c "import importlib.util, pathlib; fx=pathlib.Path('src/data_utils.py'); spec=importlib.util.spec_from_file_location('fx_data_utils', fx); mod=importlib.util.module_from_spec(spec); spec.loader.exec_module(mod); assets=sorted(list(getattr(mod,'DEFAULT_ASSET_UNIVERSE'))); print(','.join(assets[:20]))"); \
...
failed: .venv/.deps_installed
	@.venv/bin/python src/purge_failed.py --list

# Purge cached data for failed assets
purge: .venv/.deps_installed
	@.venv/bin/python src/purge_failed.py $(ARGS)
...
setup:
	...
	@echo "============================================================"
	@.venv/bin/python src/precache_data.py --workers 2 --batch-size 16 || true
	@echo ""
	@echo "============================================================"
	...
	@echo "============================================================"
	@.venv/bin/python src/clean_cache.py
	@echo ""
	@echo "============================================================"
